...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.delegage && [self.delegage respondsToSelector:@selector(richTextView: touchBeginRun:)])
    {
        ...
        if (self.runRectDictionary.count == 0 || !self.runRectDictionary) {
             [super touchesBegan:touches withEvent:event];
        }
    NSSet *set =  [self.runRectDictionary keysOfEntriesPassingTest:^BOOL(id key, id obj, BOOL *stop) {
...
//        [self.runRectDictionary enumerateKeysAndObjectsWithOptions:NSEnumerationReverse usingBlock:^(id key, id obj, BOOL *stop) {
//            CGRect rect = [((NSValue *)key) CGRectValue];
//            if(CGRectContainsPoint(rect, runLocation))
//            {
//                self.touchRun = obj;
